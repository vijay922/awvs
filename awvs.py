...
def push_wechat_group(content):
    ...
    try:
        # print('Start pushing')
        # This is modified to the webhook address of your own robot
            ...
            try:
                ...
                if high_count!=init_high_count:
                    ...
                    for xxx in result['most_vulnerable_targets']:
                        ...
                        message_push=message_push+'target:'+xxx['address']+'\n'

                    for xxxx in result['top_vulnerabilities']:
                        message_push = message_push+'vulnerability: ' + xxxx['name'] + 'quantity: '+str(xxxx['count'])+'\n'                    push_wechat_group(message_push)

                    init_high_count=high_count
                    message_push=''
                else:
                    #print('The number of high-risk vulnerabilities has not changed',high_count)
                    init_high_count = high_count
            except Exception as e:
...
""")
    ...
    try:
        ...
        if target_scan==False:
            if '8'==scan_type:
                is_to_scan = False
        profile_id = mod_id[scan_type] # Get scan vulnerability type
        if '9' == scan_type:
    ...
    if target_scan==False:
            ...
            if target:
                ...
                try:
                    if target_state[0]==1:
                        ...
                        print("{0} has been added to the scan queue, the first: ".format(target),str(add_count_suss))
    ...
    elif target_scan==True: #Scan an existing target
        ...
        for target_for in get_target_list():
            ...
            configuration(awvs_url, target_for['target_id'], target_for['address'],profile_id) #Configuration when there is a target scan
            try:
